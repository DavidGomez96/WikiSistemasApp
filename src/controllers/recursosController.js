...
export class RecursoController {
  ...
  static async createRecurso (req, res) {
    try{
      const { descripcion, materia_id, tipo_recurso_id } = req.body;
      const newRecurso = await Recurso.create({ descripcion, materia_id, tipo_recurso_id });
      res.json(newRecurso);
    }catch (error) {
      res.status(400).json({ error: error.message })
    }
  ...
  static async updateRecurso (req, res) {
    try{
      const { id } = req.params;
      const { descripcion, materia_id, tipo_recurso_id } = req.body;
      await Recurso.update({ descripcion, materia_id, tipo_recurso_id }, { 
        where: { recurso_id: id }
      })
